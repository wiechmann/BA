...
std::string Result::toCSV() const {
    ...
    out << dataset << ","
        ...
        << compressionTime << ","
        << decompressionTime << ","
        << (shuffleTime + compressionTime + decompressionTime);
    return out.str();
}

std::string Result::csvHeader() {
    return "dataset,compressor,level,size,compressed size,ratio,shuffle time,compression time,decompression time,time\n";
}
...
Result benchmark(const std::unique_ptr<Compressor>& compressor,
    ...
    return { "",
        ...
        compressedSize,
        shuffleTime / repeat,
        compressionTime / repeat,
        decompressionTime / repeat };
}
